...
def rooms_360(look_at_fn, radius=1, num_samples=200, altitude=45):
    ...
    for angle in np.linspace(0, 2 * np.pi, num_samples):
        translation = np.array(
            [virtual_radius * np.sin(angle), 0.75, virtual_radius * np.cos(angle)]
        )
        ...
        cam2world[1, 3] = 0.75
        cam2world[:3, 1] = torch.Tensor([0.0, -1.0, 0.0])
...
def around(look_at_fn, radius=1, num_samples=200, altitude=45):
    ...
    for angle in np.linspace(0, 2 * np.pi, num_samples):
        translation = np.array(
            [virtual_radius * np.sin(angle), virtual_radius * np.cos(angle), z_coord]
        )
        cam2world = look_at_fn(translation, np.array([0.0, 0.0, 0.0]))
        cam2world = torch.from_numpy(cam2world).float()
        trajectory.append(cam2world)
...
def back_and_forth(look_at_fn, radius=1, num_samples=200, altitude=0):
    ...
    for distance in distances:
        translation = (
            np.array([virtual_radius * np.sin(0), virtual_radius * np.cos(0), z_coord])
            * distance
        )
        cam2world = look_at_fn(translation, np.array([0.0, 0.0, 0.0]))
        cam2world = torch.from_numpy(cam2world).float()
