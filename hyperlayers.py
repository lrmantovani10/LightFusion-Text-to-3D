...
class LowRankHyperNetwork(nn.Module):
    def __init__(
        ...
        for name, param in self.hypo_parameters.items():
            ...
            out_features = (
                int(torch.prod(torch.tensor(param.size())))
                if "bias" in name
                else param.shape[0] * rank + param.shape[1] * rank
            )
            ...
            hn = custom_layers.FCBlock(
                ...
                hidden_ch=hyper_hidden_features,
            ...
            if "bias" in name:
                ...
                hn.net[-1].bias.data = torch.ones_like(hn.net[-1].bias.data) / np.sqrt(
                    self.rank
                )
    ...
    def forward(self, z):
        ...
        for name, net, param_shape in zip(self.names, self.nets, self.param_shapes):
            ...
            if "bias" in name:
                ...
                params[name] = self.hypo_parameters[name] + low_rank_params.reshape(
                    batch_param_shape
                )
            else:
                a = low_rank_params[:, : self.rank * param_shape[0]].view(
                    -1, param_shape[0], self.rank
                )
                b = low_rank_params[:, self.rank * param_shape[0] :].view(
                    -1, self.rank, param_shape[1]
                )
...
class HyperNetwork(nn.Module):
    def __init__(
        ...
        for name, param in hypo_parameters:
            ...
            if "weight" in name:
                hn.net[-1].apply(
                    lambda m: hyper_weight_init(m, param.size()[-1], siren=True)
                )
            elif "bias" in name:
